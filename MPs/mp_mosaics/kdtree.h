...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     unsigned partition(vector<Point<Dim>>& list, int d, unsigned left, unsigned right, unsigned pivotIndex);

     Point<Dim>& quickSelect(vector<Point<Dim>>& list, int d, unsigned left, unsigned right, unsigned k);

     KDTreeNode* makeTreehelper(vector<Point<Dim>>& values, int d, unsigned left, unsigned right);

     Point<Dim> findNearestNeighbor(KDTreeNode * subroot, const Point<Dim>& query, int d) const;

     void deletify(KDTreeNode * subroot);
     void copierhelper(KDTreeNode * subroot, KDTreeNode * othersubroot);
};
